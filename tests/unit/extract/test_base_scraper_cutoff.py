...
class TestCutoffLogic:
    ...
    def test_extract_skips_stale_listings(self, monkeypatch) -> None:
        ...
        class TestScraper(MockScraper):
            def crawl_list_pages(self):
                return iter([
                    {"url": "http://fresh.com", "source_listing_id": "1", "title": "Fresh", "post_date": fresh_date},
                    {"url": "http://stale.com", "source_listing_id": "2", "title": "Stale", "post_date": stale_date},
                ])

        scraper = TestScraper(source_id=1)
        with patch("korean_rental_etl.load.audit.start_run") as mock_start, \
    ...
    def test_extract_keeps_listings_with_none_post_date(self, monkeypatch) -> None:
        ...
        class TestScraper(MockScraper):
            def crawl_list_pages(self):
                return iter([
                    {"url": "http://unknown.com", "source_listing_id": "1", "title": "Unknown Date", "post_date": None},
                ])
